...
bool p_isLeapYear(_In_ int year) {
    return (year % 4 == 0 && year % 100 != 0) || year % 400 == 0;
}

bool p_checkDate(_In_ int year, _In_ int month, _In_ int day, _In_ bool checkAllYear) {
    // all year -> [1600, 6399]
    if (checkAllYear) {
...
int p_getDayOrdinal(_In_ int year, _In_ int month, _In_ int day) {
    int ordinal;
...
int p_getDateFromOrdinal(_In_ int year, _In_ int ordinal, _Out_  int *month, _Out_ int *day) {
...
int p_getDaysOfMonth(_In_ int year, _In_ int month) {
    int days = 0x3f3f3f3f;
    if (month == 1 || month == 12) {
        days = 31;
    } else if (month == 2) {
...
int p_getDayOfWeek(_In_ int year, _In_ int month, _In_ int day) {
...
int p_getMonthInfo(_In_ int lunarYear) {
...
int p_getLunarLeapMonth(_In_ int lunarYear) {
    return (p_getMonthInfo(lunarYear) >> 13) & 0x0F;
}

int p_getLunarNewYearOrdinal(_In_ int lunarYear) {
    return (p_getMonthInfo(lunarYear) >> 17) & 0x3F;
}

int p_getLunarDaysOfMonth(_In_ int lunarYear, _In_ int lunarMonth, _In_ bool leapMonth) {
    int DX_data;//该年大小月情况
    int Acc_LeapMonth;
...
bool p_lunarExpandDX(_In_ int lunarYear, _Out_ int daysOfMonth[15]) {
    int pos = 0;//数组写入位置
    unsigned int leapMonth;//闰月
    memset(daysOfMonth, 0, 15 * sizeof(int));//清空数组
...
bool p_getJieqiData(_In_ int year, _In_ int month, _Out_ int jieqiDate[2], _Out_ int jieqiOrdinal[2]) {
...
SolarInfo p_getSolarInfo(_In_ int year, _In_ int month, _In_ int day) {
    SolarInfo solarDate;
...
LunarInfo p_getLunarInfo(_In_ int year, _In_ int month, _In_ int day) {
...
JieqiInfo p_getJieqiInfo(_In_ int year, _In_ int month, _In_ int day) {
...
Calendar p_getCalendarInfo(_In_ int year, _In_ int month, _In_ int day) {
    Calendar calendar;
...
bool getDayOrdinal(_In_ int year, _In_ int month, _In_ int day, _Out_ int *ordinal) {
    if (ordinal) {
...
bool getDateFromOrdinal(_In_ int year, _In_ int ordinal, _Out_ int *month, _Out_ int *day) {
    if (month && day) {
...
bool getDaysOfMonth(_In_ int year, _In_ int month, _Out_ int *days) {
    if (days) {
...
bool getDayOfWeek(_In_ int year, _In_ int month, _In_ int day, _Out_ int *week) {
    if (week) {
        if (p_checkDate(year, month, day, true)) {
...
bool getSolarInfo(_In_ int year, _In_ int month, _In_ int day, _Out_ SolarInfo *solarInfo) {
    if (solarInfo) {
        if (p_checkDate(year, month, day, false)) {
            *solarInfo = p_getSolarInfo(year, month, day);
            return true;
        } else {
            memset(solarInfo, 0x3f, sizeof(SolarInfo));
        }
    }
    return false;
}

bool getLunarInfo(_In_ int year, _In_ int month, _In_ int day, _Out_ LunarInfo *lunarInfo) {
    if (lunarInfo) {
        if (p_checkDate(year, month, day, false)) {
            *lunarInfo = p_getLunarInfo(year, month, day);
            return true;
        } else {
            memset(lunarInfo, 0x3f, sizeof(LunarInfo));
        }
    }
    return false;
}

bool getJieqiInfo(_In_ int year, _In_ int month, _In_ int day, _Out_ JieqiInfo *jieqiInfo) {
    if (jieqiInfo) {
        if (p_checkDate(year, month, day, false)) {
            *jieqiInfo = p_getJieqiInfo(year, month, day);
            return true;
        } else {
            memset(jieqiInfo, 0x3f, sizeof(JieqiInfo));
        }
    }
    return false;
}

bool getCalendarInfo(_In_ int year, _In_ int month, _In_ int day, _Out_ Calendar *calendar) {
    if (calendar) {
        if (p_checkDate(year, month, day, false)) {
            *calendar = p_getCalendarInfo(year, month, day);
            return true;
        } else {
            memset(calendar, 0x3f, sizeof(Calendar));
        }
    }
    return false;
}
